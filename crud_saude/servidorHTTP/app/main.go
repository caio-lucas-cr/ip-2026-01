...
import (
	...
	"strings"                   // Usado para filtrar endereços APIPA
)
...
func main() {
	...
	http.HandleFunc("/patientDetail", handlers.PatientDetailHandler)
	...
	// Rotas para laudos médicos
	http.HandleFunc("/createMedicalReport", handlers.CreateMedicalReportHandler)
	http.HandleFunc("/editMedicalReport", handlers.EditMedicalReportHandler)
	http.HandleFunc("/deleteMedicalReport", handlers.DeleteMedicalReportHandler)
	http.HandleFunc("/profile.html", handlers.ProfileHandler)
	...
	if localIP == "" || strings.HasPrefix(localIP, "169.254.") {
		localIP = "127.0.0.1"
	}

	// Imprime no terminal o endereço e a porta onde o servidor está rodando
	fmt.Printf("Servidor rodando em: http://localhost:%s/ (ou http://%s:%s/)\n", port, localIP, port)
