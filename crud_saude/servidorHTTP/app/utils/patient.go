...
func GetAllPatients() ([]Patient, error) {
	query := `SELECT id, name, email, cpf, born_date, phone, address, created_at, updated_at FROM patients ORDER BY created_at DESC`
	...
	for rows.Next() {
		var patient Patient
		err := rows.Scan(&patient.ID, &patient.Name, &patient.Email, &patient.CPF, &patient.BornDate, &patient.Phone, &patient.Address, &patient.CreatedAt, &patient.UpdatedAt)
		if err != nil {
...
func GetPatientByID(id int) (*Patient, error) {
	query := `SELECT id, name, email, cpf, born_date, phone, address, created_at, updated_at FROM patients WHERE id = $1`
	var patient Patient
	err := DB.QueryRow(query, id).Scan(&patient.ID, &patient.Name, &patient.Email, &patient.CPF, &patient.BornDate, &patient.Phone, &patient.Address, &patient.CreatedAt, &patient.UpdatedAt)
	if err != nil {
...
func UpdatePatient(id int, name, email, cpf, bornDate, phone, address string) error {
	query := `UPDATE patients SET name = $1, email = $2, cpf = $3, born_date = $4, phone = $5, address = $6, updated_at = CURRENT_TIMESTAMP WHERE id = $7`
